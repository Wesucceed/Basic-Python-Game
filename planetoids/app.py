...
class Planetoids(GameApp):
    ...
    def start(self):
        ...
        if self._state == STATE_ACTIVE:
            self._message = None
        self._message.top = (self._title.bottom)/2 + 100
    ...
    def update(self,dt):
        ...
        """
        assert isinstance(dt, (int, float)), repr(dt)+ ' is not a number'
        self._s_pressed_current()
        if self._state == STATE_INACTIVE:
            self._gameInactive()
        if self._state == STATE_ACTIVE:
            self._gameActive()
    ...
    def _gameActive(self):
        ...
        if self._wave.gameLost():
            self._state = STATE_COMPLETE
            self._message = GLabel(text = "You lost",
                                font_name = MESSAGE_FONT,
                                font_size = MESSAGE_SIZE,
                                x = self.width/2,
                                y = self.height/2)
        if self._wave.gameWon():
            self._state = STATE_COMPLETE
            self._message = GLabel(text = "You Won",
                                font_name = MESSAGE_FONT,
                                font_size = MESSAGE_SIZE,
                                x = self.width/2,
                                y = self.height/2)
    ...
    def _gameInactive(self):
        ...
        if self._sdown:
                ...
                self._message = None
                self._lastkeys = 0
                wave = GameApp.load_json(DEFAULT_WAVE)
                self._state = STATE_ACTIVE
